...
for k, (i, isl) in enumerate(gdf.iterrows(), 1):
    ...
    if (k-1)%10==0 and k<2950 and k>801 and k<818:
        ...
        #creo la maschera per i vari valori non validi, la applico ed estraggo la geometria
...
for h in range(len(excluded_values)):
    print(excluded_values[h], np.mean(gdf[f'terreni {h}']))