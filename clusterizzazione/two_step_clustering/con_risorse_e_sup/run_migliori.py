...
colonne_includere=['Densità_pop', 'gdp_pro_capite', 'temp', 'gdp_pop_urban_merged', 'geothermal_potential', 'hydro', 'superficie_res']
...
for method in colonne_clusters:
    ...
    colonne = [col for col in df2.columns if col not in colonne_etichette+colonne_risorse+colonne_varianza+colonne_identificative]
    ...
    colonne = [col for col in df3.columns if col not in colonne_etichette+colonne_risorse+colonne_varianza+colonne_identificative]
