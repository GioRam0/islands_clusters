...
from active_semi_clustering.semi_supervised.pairwise_constraints import RCAKMeans
...
for k in cluster_range:
    #RCAKMeans, problema matrice covarianze non invertibile
    rcak = RCAKMeans(n_clusters=k)
    rcak.fit(df[colonne_includere].values, ml=ml, cl=cl)
    score_rca = silhouette_score(df[colonne_includere], rcak.labels_)
    df[f'cluster_label_rca_{k}']=rcak.labels_
    shilouette.append(score_rca)
