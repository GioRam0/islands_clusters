...
def funzione(stringa):
    ...
    #conservo solo le shapes che intersecano almeno un'isola
    gdf1=gdf1[gdf1['isole_associate'].apply(lambda x: len(x) > 0)]
    ...
    for cont,(j,shape) in enumerate(gdf1.iterrows(),1):
        ...
        unione_isole=riproietta_poligono(gdf.loc[indici_isole[0],'geometry'],crs)[0]
        ...
        #se troppo poca superficie proseguo
        if area_unione<1000:
            continue
        ...
        if len(indici_isole)==1:
            ...
            isola1=riproietta_poligono(gdf.loc[indici_isole[0],'geometry'])[0].intersection(shape_ripro)
            isola2=riproietta_poligono(gdf.loc[indici_isole[1],'geometry'])[0].intersection(shape_ripro)
            ...
            for ind in range(len(indici_isole)):
                isola=riproietta_poligono(gdf.loc[indici_isole[ind],'geometry'], crs)[0].intersection(shape_ripro)
